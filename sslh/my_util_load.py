...
def load_adjacency_matrix_versiondetect(data):
	try:
		#f=open(adjMatPath,"r")
		#data = p.load(f)
		size = data[-1][0] + 1
...
def load_prior_belief_versiondetect(allLabelNamePath, seedlabelPath):
	try:
		allLabelName = p.load(open(allLabelNamePath,'r'))

		lines = p.load(open(seedlabelPath, 'r'))
		c = np.zeros(shape=(len(lines),len(allLabelName)))

		for idx, line in enumerate(lines):
			index2 = allLabelName.index(line[0])
			c[idx,index2] = int(line[1])
		print c[0]
		return c
	except IOError as error:
		raise error
