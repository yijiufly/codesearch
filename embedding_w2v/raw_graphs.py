...
class raw_graph:
    ...
    # To generate acfg matrix and its neighbor matrix
    def gen_matrix(self):
        # creating temp list of list which needs to be converted into matrix form
        for node in self.g.nodes():
            self.fv_list.append(self.g.node[node]['v'])
        # list of egdes from which neighbour matrix need to be created
    ...
    def retrieveVec(self, id_, g):
        ...
        feature_vec.append(offs)
        #betweens7
        between = retrieveGP(g)
        feature_vec.append(between)
        print feature_vec
        return feature_vec
        '''
...
class CallGraph:
    ...
    def __repr__(self):
        ret = ''
        for node in self.g.nodes():
            ret += "{} {}->{}\n".format(str(node), self.fv[node], self.g.neighbors(node))
        return ret

    def add_edge_from(self, funcname, refs):
        self.g.add_node(funcname)
        for callee_name in refs:
            self.g.add_edge(funcname, callee_name)

    def add_edge_to(self, refs, funcname):
        self.g.add_node(funcname)
        for caller_name in refs:
            self.g.add_edge(caller_name, funcname)
    ...
    def cal_feature(self):
        ...
        for node in self.g.nodes():
            self.fv[node] = list()
            feature = dict()
            feature['offs'] = len(list(self.g.successors(node)))
            self.fv[node].append(feature['offs'])
            feature['desc'] = len(nx.descendants(self.g, node))
            self.fv[node].append(feature['desc'])
            feature['btwn'] = between[node]
            self.fv[node].append(feature['btwn'])
            feature['clos'] = close[node]
            self.fv[node].append(feature['clos'])
            self.f = feature
