...
class Graph(object):
    ...
    def factor(self, rvs, name='', potential=None, meta={},
        ...
        f = Factor(rvs, name, potential, meta, debug, ftype=ftype)
        self.add_factor(f)
        return f
    ...
    def count_rv_marginals(self, rvs=None, normalize=False):
        ...
        for rv, marg in tuples:
            ...
            if marg[0] < marg[1]:
                key = rv_label.split('##')
                if key[0] in prediction:
                    prediction[key[0]].append(key[1]+' '+str(marg[1]))
                else:
                    prediction[key[0]]=[key[1]+' '+str(marg[1])]
    ...
    def recompute_outgoing(self, normalize=False):
        ...
        if self._ftype == '2':
            ...
            if len(self._rvs) == 1:
                ...
                for i, rv in enumerate(self._rvs):
                    o = rv.get_outgoing_for(self)
                    if o[1] == largest:
                        o = np.array([0.001,0.999])
                    else:
                        o = np.array([0.999,0.001])
                    if normalize:
                        o = divide_safezero(o, sum(o))
                        self._outgoing[i] = o
                    convg = convg and \
