...
# Generate acfg for a specific function
# Input: function under analysis
# Output: acfg for targeted function
def getCfg(func, externs_eas, ea_externs):
	...
	for bl in keys:
		...
		#if end in seq_blocks and print_insn_mnem(PrevHead(end)) != 'jmp':
		if start == func_start:
...
# calculate statistical features for node in acfg
# input: acfg
# output: acfg with statistical features calculated
...
def gen_cfg_graph(func, path, title):
	func_start = func.start_ea
	func_end = func.end_ea
	# path = "/home/renee/Downloads/sample1.gdl"
	gen_flow_graph(path, title, func_start, func_end, CHART_GEN_GDL)
...
# Generate block sequence from function flowchart
# Input: function start point
# Output: obtain block and contain point mapping
...
# Find condition place
# Input: basic block
# Output: if find condition instruction, return; otherwise, return the end of current basic block
def checkCB(bl):
	start = bl[0]
	end = bl[1]
	ea = start
