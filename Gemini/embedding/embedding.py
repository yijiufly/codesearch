...
class Embedding:
	...
	def get_some_acfgs(self, it, cnt=35):
		mul_mat = []
		acfg_mat = []
		acfg_nbr_mat = []
		func_name_list = []
		func_name_list_with_size = []
		...
		while len(func_name_list) < cnt:
			...
			fvec_list = []
			func_name = acfg.funcname
			# if len(acfg.fv_list) > 1:
			func_name_list_with_size.append((func_name, len(acfg.fv_list)))
		...
		return acfg_nbr_mat, acfg_mat, mul_mat, func_name_list, func_name_list_with_size
	...
	def embed_a_binary(self, idafile, target_name=None):
		...
		func_name_list_with_size = []
		while True:
			acfg_nbr_mat, acfg_mat, mul_mat, name_list, name_list_with_size = self.get_some_acfgs(it)
			if len(mul_mat) == 0:
				break

			embed = self.embed_multiple_function(np.concatenate([acfg_mat, np.transpose(mul_mat)], 1), acfg_nbr_mat)

			retval.extend(embed)
			func_names.extend(name_list)
			func_name_list_with_size.extend(name_list_with_size)
		...
		return func_names, retval, func_name_list_with_size

	def embed_multiple_function(self, acfg_mat, acfg_nbr):
		# print(self.siamese.x.shape)
		# print(acfg_mat.size)
		r5 = self.tf_sess.run(self.g_embed_funcs, feed_dict={self.siamese.x: acfg_mat, self.siamese.n: acfg_nbr})
		return r5[0]
