...
class Labels:
    def __init__(self, seedlabels=None, funcNameList=None, finalLabels=None, groundTruth=None, alllabels=None):
        ...
        self.finalLabels = finalLabels #final labels is a N*M np array, M is the total amount of labels
    ...
    def compareOutWithGroundTruth(self):
        ...
        for idx, line in enumerate(self.funcNameList):
            ...
            if line != lastline and lastline != '':
                ...
                print top_3_idx
                #pdb.set_trace()
                print "predicted label" + self.alllabels[top_3_idx[0]] + ", " + self.alllabels[top_3_idx[1]] + ", " + self.alllabels[top_3_idx[2]] + ", " + str(count[top_3_idx]) + "\n"
                count = np.zeros(shape=(1, len(self.alllabels)))
                binaryidx += 1
			#for every function find the 3 largest non zero values
            top_3_idx = np.argsort(self.finalLabels[idx])[-3:]
    ...
    def analyse_naive(self):
        ...
        labelof = p.load(open("data/versiondetect/test1/labelAfterFirstRound.p" ,"r"))
        addfuncNameList = open("data/versiondetect/test1/versiondetect_addfunc_list.txt", "r").readlines()
        ...
        for idx, line in enumerate(self.funcNameList):
            ...
            for knn in kNNs:
                if knn[2] > 0.9999 and labelof[knn[1][0]] != [] and knn[0][0] - len(labelof) == idx:
                    ...
                    count[labelof[knn[1][0]][0]] += 1.0/strong_label_count[labelof[knn[1][0]][0]]
                    labelof[knn[1][0]] = []
            lastline = line
